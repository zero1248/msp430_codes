/*
 * I2C.c for EEPROM(more strict than others)
 *        Tips: modificate the slave address by yourself
...
/********************************************************
*名       称：I2C_Master_NoAsk()
*功       能：主机无应答
*入口参数：无
*出口参数：无
*说       明：主机接受最后一个字节后做出非应答(1)表示数据传输完成
********************************************************/
void I2C_Master_NoAsk(void)
{
	I2C_DATA_HIGH;
	DELAY_2US;
	I2C_CLK_HIGH;   //发出1应答位
...
/*******************************************
函数名称：I2C_Check_SlaveAsk()
功       能：检查从机的应答操作
入口参数：无
出口参数：1-有应答 0-无应答
说       明：
********************************************/
unsigned char I2C_Check_SlaveAsk(void)
{
	unsigned char slaveask;
	I2C_DATA_HIGH;
	DELAY_2US;
	I2C_CLK_LOW;
	delay0(15);
	I2C_CLK_HIGH;
	DELAY_2US;DELAY_2US;
	slaveask = I2C_DATA_IN;   //读入SDA数值
	I2C_CLK_LOW;
	I2C_delay_us();
	if(slaveask)    return FALSE;
	else            return TRUE;
}
...
unsigned char I2C_RxFrame(unsigned char *p_Rx , unsigned char num )
{
	...
	{
		p_Rx[i] = I2C_Get_Char();
		if( i < (num-1) ) I2C_Master_Ask();
		else    I2C_Master_NoAsk();
	}
	I2C_STOP;
	//_EINT();
	return 1;
...
void I2C_Master_NoAsk(){}//兼容软件IC
unsigned char I2C_Check_SlaveAsk(){}//兼容软件IC
